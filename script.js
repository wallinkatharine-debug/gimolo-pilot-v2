...
function renderActivity(a){
  ...
  // Kicker: keep simple + consistent with the new card
  try{
    const raw = rand(vibe.sparkHeaders[state.tone] || vibe.sparkHeaders.classic);
    const clean = String(raw).replace(/^[^A-Za-z0-9]+/,"").trim();
    $("sparkLine").textContent = (clean || "Tonight").toUpperCase();
  }catch(e){
    $("sparkLine").textContent = "TONIGHT";
  }

  $("activityTitle").textContent = a.title;
  $("activityDesc").textContent = a.description;

  // Supporting info: one calm line, pushed to the bottom
  const time =
    a.time_bucket==="5" ? "0–5 min" :
    a.time_bucket==="10" ? "10 min" : "+10 min";
  const effort =
    a.effort==="light" ? "Light" :
    (a.effort==="med" || a.effort==="medium") ? "Medium" : "High";
  const loc = cap(a.location || "either");
  const who = a.supports_solo ? "Solo" : "Group";
  $("metaLine").textContent = `${time} • ${loc} • ${effort} • ${who}`;

  $("activityPrompt").textContent = "Ready?";
...
function wireMain(){
  ...
  $("swapInProgressBtn").addEventListener("click",()=>{beep("click"); microHaptic(); const a=chooseActivity(); renderActivity(a); setScreen("screenActivity"); updateMicro();});

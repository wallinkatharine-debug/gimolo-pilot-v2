...
function renderActivity(a){
  state.current=a;
  // header line stays playful but light
  ...
  const time = a.time_bucket==="5" ? "0–5 min" : a.time_bucket==="10" ? "10 min" : "+10 min";
  const effort = a.effort==="light" ? "Light" : a.effort==="med" ? "Medium" : "High";
  const loc = a.location ? cap(a.location) : "Either";
  const who = a.supports_solo ? "Solo" : "Group";
  $("metaLine").textContent = `${time} • ${loc} • ${effort} • ${who}`;
...
function wireNav(){
  ["navFilters"].forEach(id=> { const el=$(id); if(el) el.addEventListener("click",()=>{beep("click"); openSheet();}); });
  ["navSpin"].forEach(id=> { const el=$(id); if(el) el.addEventListener("click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();}); });
  ["navProfile","profileBtn"].forEach(id=> { const el=$(id); if(el) el.addEventListener("click",()=>{beep("click"); openProfile();}); });
}
...
function wireMain(){
  ...
  const sa=$("spinAgainText");
  if(sa){
    sa.addEventListener("click",async ()=>{ beep("click"); microHaptic(); await spinSuspense(false); });
    sa.addEventListener("keydown",async (e)=>{ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); beep("click"); microHaptic(); await spinSuspense(false);} });
  }

  const back=$("backBtn"); if(back) back.addEventListener("click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();});

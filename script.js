...
const on = (id, evt, fn)=>{ const el = $(id); if(!el) return; el.addEventListener(evt, fn); };
...
function wireSheet(){
  on("communityOnly","change",(e)=>{state.filters.communityOnly=e.target.checked; persist(); applyFilters();
  initSparkles();});
  on("soloOnly","change",(e)=>{state.filters.soloOnly=e.target.checked; persist(); applyFilters();});
  ...
  refreshSegUI();
  on("applyBtn","click",()=>{beep("click"); closeSheet();});
  on("closeSheetBtn","click",()=>{beep("click"); closeSheet();});
...
function wireMain(){
  ...
  on("spinAgainBtn","click",async ()=>{
    beep("click"); microHaptic();
    await spinSuspense(false);
  });

  on("backBtn","click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();});
  on("startBtn","click",(e)=>{const p=centerOf(e.currentTarget); sparkleBurst(p.x,p.y,"spark"); microHaptic(); startActivity();});

  on("doneBtn","click", doneActivity);
  on("progressBackBtn","click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();});

  on("doneSpinBtn","click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();});

  on("profileBackBtn","click",()=>{beep("click"); setScreen("screenSpin"); updateMicro();});

  on("keepBtn","click", async ()=>{
    beep("click"); microHaptic(); toast("Keeping this energy.");
    setScreen("screenSpin");
    await wait(220);
    await spinSuspense(true);
  });
  on("switchBtn","click", async ()=>{
    beep("click"); microHaptic();
